...
function selectReportRows(transactions, mode) {
    ...
    const cashflowTypes = ['tahsilat', 'ödeme', 'odeme']; // ONLY actual cash movements (no transfers)
...
/**
 * Auto-generate contextual description for cash flow transactions
 * @param {Object} tx - Transaction object
 * @param {string} reportMode - Current report mode ('income' or 'cashflow')
 * @returns {Object} { description: string, isAutoGenerated: boolean }
 */
function generateCashFlowDescription(tx, reportMode) {
    const txType = String(tx.islemTipi || '').toLowerCase().trim();
    const existingDesc = (tx.aciklama || '').trim();
    
    // Only auto-generate for cash flow mode and specific transaction types
    if (reportMode !== 'cashflow') {
        return { description: existingDesc || '-', isAutoGenerated: false };
    }
    ...
    // Only auto-generate for tahsilat and odeme
    const shouldAutoGenerate = txType === 'tahsilat' || txType === 'ödeme' || txType === 'odeme';
    ...
    const sourceAccount = getAccountName(tx.kaynakCari || tx.islemCari);
    const targetAccount = getAccountName(tx.hedefCari);
    const amount = Math.abs(Number(tx.toplamTutar || tx.tutar || 0));
    const invoiceNo = tx.faturaNumarasi || tx.faturaNo;
    ...
    if (txType === 'tahsilat') {
        ...
        if (sourceAccount && targetAccount) {
            description = `Tahsilat: ${sourceAccount}'dan ${targetAccount}'a ${formatCurrency(amount)}`;
        } else if (sourceAccount) {
            description = `Tahsilat: ${sourceAccount}'dan ${formatCurrency(amount)}`;
        } else {
            description = `Tahsilat: ${formatCurrency(amount)}`;
        }
        
        // Add invoice number if exists
        if (invoiceNo) {
            description += ` (Fatura No: ${invoiceNo})`;
        }
    } else if (txType === 'ödeme' || txType === 'odeme') {
        // Outgoing payment
        if (sourceAccount && targetAccount) {
            description = `Ödeme: ${sourceAccount}'tan ${targetAccount}'e ${formatCurrency(amount)}`;
        } else if (targetAccount) {
            description = `Ödeme: ${targetAccount}'e ${formatCurrency(amount)}`;
        } else if (sourceAccount) {
            description = `Ödeme: ${sourceAccount}'tan ${formatCurrency(amount)}`;
        } else {
            description = `Ödeme: ${formatCurrency(amount)}`;
        }
    ...
    if (!description && sourceAccount && targetAccount) {
        description = `${sourceAccount} → ${targetAccount} (${formatCurrency(amount)})`;
    }
    ...
    return { 
        ...
        isAutoGenerated: !!description 
    };
}

/**
 * Get signed amount display based on report type and transaction type
 * @param {Object} tx - Transaction object
 * @param {string} reportMode - Current report mode ('income' or 'cashflow')
 * @returns {string} Sign prefix ('+', '–', or '')
 */
function getAmountSign(tx, reportMode) {
    const txType = String(tx.islemTipi || '').toLowerCase().trim();
    const amount = Math.abs(Number(tx.toplamTutar || tx.tutar || 0));
    
    // No sign for zero or null values
    ...
    if (reportMode === 'income') {
        // Gelir / Gider (P&L) Mode
        if (txType === 'gelir') {
            return '+';
        } else if (txType === 'gider') {
            return '–';
        } else if (isDebtTransfer) {
            return '–'; // liability increase
        }
    } else {
        // Nakit Akışı (Cash Flow) Mode - ONLY tahsilat and ödeme
        if (txType === 'tahsilat') {
            return '+';
        } else if (txType === 'ödeme' || txType === 'odeme') {
            return '–';
        }
        // Note: transfers are excluded at filter level, won't appear here
    }
    
    // Fallback: no sign for unrecognized types
    return '';
}
...
function renderTransactionTable() {
    ...
    try {
        ...
        sorted.forEach((tx, index) => {
            ...
            // Get sign for amount display
            const amountSign = getAmountSign(tx, state.reportMode);
            const formattedAmount = amountSign ? `${amountSign} ${formatCurrency(amount)}` : formatCurrency(amount);
            
            // Get description - special handling for debt transfers and cash flow auto-generation
            let description;
            let isAutoGenerated = false;
            let fullDescription = ''; // For tooltip
            
            if (isDebtTransfer && tx.kaynakCari && tx.hedefCari) {
                // Debt transfers: use arrow notation
                const sourceAccount = getAccountName(tx.kaynakCari);
                const targetAccount = getAccountName(tx.hedefCari);
                description = `${sourceAccount} → ${targetAccount}`;
                fullDescription = description;
            } else {
                // Use auto-generation for cash flow transactions
                const descResult = generateCashFlowDescription(tx, state.reportMode);
                description = descResult.description;
                isAutoGenerated = descResult.isAutoGenerated;
                fullDescription = tx.aciklama || description; // Use original for tooltip if exists
            }
            
            // Build description HTML with tooltip and [auto] tag
            let descriptionHTML = '';
            if (description.length > 120 || isAutoGenerated) {
                // Add tooltip for long or auto-generated descriptions
                const tooltipText = fullDescription || description;
                descriptionHTML = `<span class="text-sm text-gray-300" title="${tooltipText.replace(/"/g, '&quot;')}">${description}</span>`;
                if (isAutoGenerated) {
                    descriptionHTML += ` <span class="text-xs text-gray-500 ml-1">[auto]</span>`;
                }
            } else {
                descriptionHTML = `<span class="text-sm text-gray-300">${description}</span>`;
            }
            ...
            row.innerHTML = `
                ...
                <td class="px-4 py-3 text-sm text-[#E8E9EB]">${descriptionHTML}</td>
                <td class="px-4 py-3 text-sm font-bold ${amountClass} text-right whitespace-nowrap" title="Bu tutar rapor türüne göre yönlendirilmiştir.">${formattedAmount}</td>
...
/**
 * Phase 2: Export to XLSX (Excel format)
...
function handleExportCSV() {
    ...
    try {
        // Export using XLSX format (matches Dashboard exactly)
        exportFinancialReportToXLSX({
            transactions: filtered,
            getAccountName: getAccountName,
            getTransactionTypeLabel: getTransactionTypeLabel,
            formatTransactionDate: formatTransactionDate,
            generateCashFlowDescription: generateCashFlowDescription,
            reportMode: state.reportMode,
            filename: filename
        });
        
        if (typeof currentDeps.showToast === 'function') {
            currentDeps.showToast(`${filtered.length} işlem başarıyla Excel formatında dışa aktarıldı.`);
        }
    } catch (error) {
        console.error('[Financial Report] XLSX export error:', error);
        if (typeof currentDeps.showToast === 'function') {
            currentDeps.showToast('Excel dışa aktarma sırasında bir hata oluştu.', true);
        }
    }
}
